...
__all__ = [
    ...
    'get_selected_value_from_element',
    'get_select_from_element',
    'str_num_to_float',
    'conciliar_transacciones'
]
...
def get_value_from_element(element: WebElement):
    try:
        elem = element.find_element_by_xpath('center/input')
...
def get_select_from_element(element: WebElement):
    elem = element.find_element_by_xpath('center/select')
    return Select(elem)
...
class ConciliacionesTable:
    ...
    def __init__(self, driver: webdriver.Chrome):
        self.driver = driver
        self.table = driver.find_element_by_xpath("//*[@id='Form:panelTabla']/table")
        self.head = self.table.find_element_by_xpath("thead")
        # info('THEAD ')
        # info(self.head.text)
        self.headers = self.head.find_elements_by_xpath("tr/th")
        # info('THEADS CONTENT')
        # info(self.headers)
        self.body = self.table.find_element_by_xpath("tbody")
        # info('TBODY CONTENT')
        # info(self.body.text)
    ...
    def conciliar(self, movimientos):
        ...
        while next_page:
            ...
            next_btn = self.driver.find_elements_by_xpath(
        ...
        info(save)
        if save:
            guardar_btn = self.driver.find_element_by_id('Form:btnGuardar')
            guardar_btn.click()
            sleep(2)
            aceptar_btn = self.driver.find_element_by_id('formMensajes:j_id385')
            aceptar_btn.click()

    def get_rows_pos(self):
        rows = self.driver.find_elements_by_xpath("//*[@id='Form:panelTabla']/table/tbody/tr")
